...
[ "$PRODUCER_STATUS" -eq 0 ] && pass "Производитель завершился без ошибки" || fail "Производитель завершился с ошибкой"
...
grep -q "Все наборы обработаны" "$PRODUCER_LOG" && pass "Все наборы обработаны" || fail "Нет подтверждения обработки всех наборов"
...
DUPLICATES="$(cat "$CONSUMER1_LOG" "$CONSUMER2_LOG" "$CONSUMER3_LOG" | sed -n 's/.*offset=\([0-9][0-9]*\).*/\1/p' | sort | uniq -d | wc -l)"
[ "$DUPLICATES" -eq 0 ] && pass "Нет повторной обработки одного блока" || fail "Один блок обработан несколькими потребителями"

grep -q "Разделяемая память и семафор удалены" "$PRODUCER_LOG" && pass "Производитель удалил IPC-объекты" || fail "Нет подтверждения удаления IPC"
