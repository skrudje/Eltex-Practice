...
static int unregister_consumer(IpcContext *context)
{
    SharedHeader *header = context->memory;
    if (semaphore_lock(context->sem_id) == -1) return -1;
    if (header->consumers_count > 0) --header->consumers_count;
    semaphore_unlock(context->sem_id);
    return 0;
}
                             ...
                             int *should_finish)
{
    ...
    *should_finish = 0;
    ...
    if (header->generation_done) *should_finish = 1;
    semaphore_unlock(context->sem_id);
    return 0;
}
...
int run_consumer(void)
{
    ...
    for (;;) {
        size_t offset;
        int minimum = 0, maximum = 0, count = 0, should_finish = 0;
        int result = process_one_block(&context, &offset, &minimum, &maximum,
                                       &count, &should_finish);
        if (result == -1) {
            ipc_detach(&context);
            return 1;
